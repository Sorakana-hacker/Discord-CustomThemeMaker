...
layout = [
    ...
    [sg.Text('Description', size=(13, 1)), sg.InputText('', size=(20, 1), key='description')],
    [sg.Text('Version', size=(13, 1)), sg.InputText('', size=(5, 1), key='ver')],
    [sg.Text('YourID', size=(13, 1)), sg.InputText('', size=(20, 1), key='id')],
...
while True:
    ...
    if event == 'go': 
        ...
        data_lines = data_lines.replace("ThemeDescription", values['description'])
        data_lines = data_lines.replace("Ver", values['ver'])
        data_lines = data_lines.replace("YourID", values['id'])
        data_lines = data_lines.replace("YourURL", values['url'])

        with open(file_name, mode="w", encoding="cp932") as f:
           f.write(data_lines)

        os.rename("Theme1.txt", values['theme']) 
